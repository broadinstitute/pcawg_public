...
if [ ! -f $bamdir/$oxoQ ]; then
    ...
    java -Xmx36000m -jar  ${Dir}/CollectOxoGMetrics.version.1.501.jar INPUT=${BAM} OUTPUT=${OUT}/${oxoQ} REFERENCE_SEQUENCE=${REF} DB_SNP=${DBSNP}  MINIMUM_QUALITY_SCORE=20 MINIMUM_MAPPING_QUALITY=30 MINIMUM_INSERT_SIZE=60 MAXIMUM_INSERT_SIZE=600 USE_OQ=true CONTEXT_SIZE=1 STOP_AFTER=2147483647 VERBOSITY=INFO QUIET=false VALIDATION_STRINGENCY=LENIENT COMPRESSION_LEVEL=5 MAX_RECORDS_IN_RAM=500000 CREATE_INDEX=false CREATE_MD5_FILE=false
    if [ $? -ne 0 ] ; then
        exit 1
    fi
else
